...
## R Markdown
When you click the **Knit** (â‡§âŒ˜K) button a document will be generated that includes the output in the document. 
...
FigS1.tif <- ggplot(Table2S, aes(Nasal_RIN, FP_RIN)) +geom_point()+  geom_text_repel(label=Table2S$subjectid) + geom_point() + xlab("Nasal RIN")+geom_smooth(method='lm')+ ylab("Fungiform RIN") + theme_bw()
FigS1.tif

postscript(colormodel="cmyk")
ggsave(filename ='S1Figurev5.tif', FigS1.tif, width = 16.93, height = 10.85, units ="cm",dpi = 1200, device='tiff', limitsize = TRUE, compression = "lzw")
dev.off()
...
##Supplemental Figure 3 correlation between qPCR and RNAseq for FP
```{r figure, include=FALSE}
...
postscript(colormodel="cmyk")  
ggsave(filename ='S3Figurev5.tif', p4, width = 15.85, height = 9.5, units ="cm",dpi = 1200, device='tiff', limitsize = TRUE, compression = "lzw")
dev.off()    
...
postscript(colormodel="cmyk")
x <-ggarrange(g9, g7, g8, 
          labels = c("A", "B", "C"), label.x = 0.95, label.y = 0.15, ncol = 1, nrow = 3)

ggsave(filename ='Figure3v5.tif', x, width = 13.12, height = 18.41, units ="cm",dpi = 1200, device='tiff', limitsize = TRUE, compression = "lzw")
dev.off()
```


#Supplemental Figure 2 plot with all subjects even those with no GNAT3
```{r figure, include=FALSE}
...
postscript(colormodel="cmyk")
y<-ggarrange(g9, g7, g8, 
          labels = c("A", "B", "C"), label.x = 0.95, label.y = 0.15, ncol = 1, nrow = 3)

ggsave(filename ='S2Figurev5.tif', y, width = 12.09, height = 18.47, units ="cm",dpi = 1200, device='tiff', limitsize = TRUE, compression = "lzw")
dev.off()
```


##Supplemental Figure 4  PTC taste in general
```{r figure, include=FALSE}
cols <- c(49, 65)
...
postscript(colormodel="cmyk")
ggsave(filename ='S4Figurev5.tif', g9, width = 13.15, height = 9.45, units ="cm",dpi = 1200, device='tiff', limitsize = TRUE, compression = "lzw")
dev.off()
```


##Supplemental Table 4  Water
```{r figure, include=FALSE}
